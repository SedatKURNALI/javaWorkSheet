...
public class FilmTahmin {
    ...
    private static void filmGetir(List<String> film, int index) {
        ...
        char harf = ' ';
        int tahminSayisi = 0;
        int dogruTahminSaisi = 0;
        String str = "";

        do {

            System.out.print("yanlıs tahmin sayisi: " + (tahminSayisi - dogruTahminSaisi) + "/" + 2 * film.get(index).length() + "\ntahmin ettiginiz harfi giriniz : ");//yanlıs girilen tahmin sayisini print edildi
        ...
        } while ((tahminSayisi - dogruTahminSaisi) != 2 * film.get(index).length());//yanlış sayısı verilen hakka ulaşana kadar do body calışır

        if ((tahminSayisi - dogruTahminSaisi) == 2 * film.get(index).length()) {
            System.out.println("yanlıs tahmin sayısı : " + (tahminSayisi - dogruTahminSaisi));
